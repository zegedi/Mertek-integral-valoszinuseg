...
%		ezért minden \( \varepsilon > 0 \)-hoz van olyan \( N \in \N \) küszöbindex, hogy
		...
		\marginnote{
			Az alkalmazott becslések részleseten:
			\begin{align*}
				\omega(f, \tau)
	...
	\[
		f_n(x) \coloneq 
		\begin{cases}
			\sqrt{n}            & \quad (0   \leq x \leq 1/n) \\[3pt]
			\dfrac{1}{\sqrt{x}} & \quad (1/n \leq x \leq 1  )
		\end{cases}
		\qquad (n \in \N).
	\]
...
%		\qquad \bigl( t \in [\xi - \delta,\, \xi + \delta] \subset \bigr).
	...
	\marginnote{
		\begin{theo*}[Lebesgue-kritérium]\label{th:lebesgue-kritérium}
			A
			\[
				g \in \Riem{[a,b]}
			\]
			...
			\[
				...
				\setc[\big]{ x \in [a,b] }{ g \notin \ContAt{x} }
			\]
			szakadási helyek halmaza nullamértékű.
		\end{theo*}
	}[-1.6\baselineskip]
